...
# SWA its own staticWebAppLocation parameter (default westus2) while
...
stages:
  - stage: BuildAndTest
    ...
    jobs:
      - template: pipelines/templates/build-backend.yml
      - template: pipelines/templates/build-frontend.yml
  ...
  - stage: CostGate
    ...
    dependsOn: InfraPlan
    jobs:
      - template: pipelines/templates/cost-gate.yml
  ...
  - stage: DeployNonProd
    ...
    jobs:
      - deployment: DeployNonProd
        ...
        environment: 'coaching-platform-nonprod'
        strategy:
          runOnce:
            deploy:
              steps:
